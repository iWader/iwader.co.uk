	...
	handle: async (
		...
		const promises = body.activities.map(async (activity: StravaActivity) => {
			if (activity.map && activity.map.summary_polyline) {
				...
				const url = new URL(`https://api.mapbox.com/styles/v1/${env.MAPBOX_USERNAME}/${env.MAPBOX_STYLE_ID}/static/${overlay}/auto/640x400@2x?access_token=${env.MAPBOX_ACCESS_TOKEN}`)
